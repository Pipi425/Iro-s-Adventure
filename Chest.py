...
def rect_distance(rect1, rect2):
    dx = max(rect1.left - rect2.right,
             rect2.left - rect1.right,
             0)

    dy = max(rect1.top - rect2.bottom,
...
class Chest:
    ...
    def update(self, player):
        ...
        distance = rect_distance(player.get_rect(), self.rect)
        keys = pygame.key.get_pressed()

        if distance < 20 and not self.opened and self.state == "idle":
    ...
    def draw_loot_ui(self, screen):
        ...
        screen.blit(
            text,
            (
                panel.right - text.get_width() - 15,
                panel.bottom - text.get_height() - 15
            )
        )
